...
3. **Workflow engine** — does the harness expose a Workflow tool (deterministic `agent()` / `pipeline()` with per-call `schema` / `model` / `effort` / `isolation`, a run journal, a token budget)? If yes, **Orchestrated mode** is available: compile the plan into the Workflow template, adapting only its `TICKETS` / seats / driver and keeping the `verifyPrompt()` builder + house-rules hardening, the `WORKER_SCHEMA`/`VERIFIER_SCHEMA` output contracts, and the blind (paths-only) verify call **verbatim** (guard an adapted copy with `verify-prompt.test.mjs <copy>` before running it) — see [references/workflow-backend.md](references/workflow-backend.md).
