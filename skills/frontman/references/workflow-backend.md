...
`templates/orchestrate.workflow.mjs` is the reference script. The frontman **copies and adapts it per run** — it is not a fixed program — but only its **run-specific regions** are adaptable: fill in `TICKETS` (or pass them as `args.tickets`), set the `SEAT` map and `CROSS_VERIFY_SEAT`, and choose the driver (sequential `for`-loop vs `pipeline()`). The **`verifyPrompt()` builder and its house-rules hardening** — the `HOUSE_RULES` coercion line, the two neutralization regexes, the `MAX` length cap — the **`WORKER_SCHEMA` / `VERIFIER_SCHEMA` output contracts**, and **`verifyStage`'s blind (paths-only) verify call** are **security-critical: keep them verbatim, never slim them when adapting**, or the `<<<…>>>` fence, forged-marker neutralization, ReDoS bounds, schema-shape enforcement, or verifier blindness silently vanish. The template's top comment marks the split (an `ADAPT vs KEEP-VERBATIM` banner, plus inline `// KEEP VERBATIM` markers directly above `verifyPrompt()`, the `HOUSE_RULES` line, each schema, and the blind call). It carries `WORKER_SCHEMA` and `VERIFIER_SCHEMA` (a scout schema can be added the same way), the seat routing, the escalation ladder, and the blind-verify prompt builder.
...
It runs all invariants against that file — the byte-identical-when-absent property, the delimited fence, forged-marker neutralization across separator variants, the length cap, the ReDoS-bounded regexes, the `WORKER_SCHEMA` / `VERIFIER_SCHEMA` output contracts (shape + enums), and `verifyStage`'s blind (paths-only) verify call — so a silently slimmed builder, a loosened schema, or a non-blind verify call is caught before the run, not after. With no arg it checks the repo template (unchanged behavior); a copy that dropped or reformatted a KEEP-VERBATIM region fails loudly with a non-zero exit.
